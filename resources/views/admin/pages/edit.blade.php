...
@endif
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('page.update',[$edit->id]) }}" enctype="multipart/form-data" method="post" id="store">
                        <input type="hidden" name="_method" value="PUT">
                        @csrf
                            <div class="form-group mb-3">
                                <label for="example-input-small">Page Title <code>*</code></label>
                                <input type="text" id="example-input-small" name="title" class="form-control form-control-sm first" placeholder="Page Title" required="" value="{{$edit->title}}">
                            </div>

                            <div class="form-group mb-3">
                                <label for="example-input-small">Page Slug <code>*</code></label>
                                <input type="text" id="example-input-small" name="slug" class="form-control form-control-sm second" placeholder="Page Slug" required="" value="{{$edit->slug}}">
                            </div>
                            ...
                            <div class="form-group mb-3">
                                <label for="example-input-normal">Country</label>
                                 ...
                                 @foreach($country as $data)
                                     <option value="{{$data->slug}}" <?php if($data->slug == $edit->countryId){ echo 'selected'; } ?>>{{$data->name}}</option>
                                @endforeach
                            ...
                            <div class="form-group mb-3">
                                <label for="example-input-normal">Category</label>
                                 ...
                                 @foreach($category as $data)
                                     <option value="{{strtolower($data->name)}}" <?php if(strtolower($data->name) == strtolower($edit->category)){ echo 'selected'; } ?>>{{$data->name}}</option>
                                @endforeach
                            ...
                            <div class="form-group mb-3">
                                <label for="example-input-small">Description <code>*</code></label>
                                 <textarea class="form-control summernote" name="description" id="long_desc" required>{{$edit->data}}</textarea>
                            </div>
